...
export default function SettingsPage() {
	...
	useEffect(() => {
		if (profileQuery.data) {
			setStart(String(profileQuery.data.startWeight));
			setGoal(String(profileQuery.data.goalWeight));
		}
	}, [profileQuery.data]);
			...
			<Card>
				...
				<CardContent className="space-y-3">
					<div className="space-y-1.5">
						...
						<Input id="s" type="number" step="0.1" value={start} onChange={(e) => setStart(e.target.value)} />
					</div>
					<div className="space-y-1.5">
						<Label htmlFor="g">目標体重 (kg)</Label>
						<Input id="g" type="number" step="0.1" value={goal} onChange={(e) => setGoal(e.target.value)} />
					</div>
